...
class App(tk.Tk):
    ...
    def load_meta(self, fn):
        self.experiments = load_experiment_metadata([fn], pre_only=self.pre_only_var.get(), log=self.log)
        self.input_label.config(text=f"{os.path.split(fn)[-1]}: {len(self.experiments)} exps")
        self.set_output_folder(os.path.join(os.path.commonpath(list(self.experiments['path'])), 'screening_results'))
        self.update_experiments_table()
    ...
    def process(self):
        ...
        for ii, (name, exp_info) in enumerate(self.experiments.iterrows()):
            ...
            try:
                the_shelldata, the_peak_table, the_powder, the_diff_img = analyze_experiment_with_cap(
                    exp_info['path'],
                    cap=self.cap,
                    keep_existing=self.keep_existing_var.get(),
