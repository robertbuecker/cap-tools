...
def main(basedir: str, print_fn=None, report_dir: Optional[str] = None):
    
    if print_fn is None:
        print_fn = print

    report_dir = basedir if report_dir is None else report_dir
    ...
    for (k, img) in imgs.items():        
        ...
        pdf_fn = os.path.join(report_dir, k + '.pdf')
...
def run_cli(
    ...
    """Run detector-distance calibration and write the CSV report.

    Defaults intentionally match the historical command-line behavior: reports
    and PDFs are written next to the selected calibration folder.
    """
    print_fn = (lambda *args, **kwargs: None) if quiet else print
    report = pd.DataFrame(main(basedir, print_fn=print_fn, report_dir=report_dir))
    report_fn = output_csv or os.path.join(basedir, 'detector_distance.csv')
    os.makedirs(os.path.dirname(os.path.abspath(report_fn)), exist_ok=True)
    report.to_csv(report_fn, float_format='%.2f')
    if not quiet:
        print('-----')
...
def main_cli(argv: Optional[List[str]] = None) -> None:
    ...
    parser = argparse.ArgumentParser(description='Calibrate detector distance from aluminum ring calibration data.')
    parser.add_argument('basedir', nargs='?', help='Folder containing CrysAlisPro calibration data sets.')
    parser.add_argument('-o', '--output-csv', help='CSV report path. Defaults to <basedir>/detector_distance.csv.')
    parser.add_argument('--report-dir', help='Directory for per-data-set PDF reports. Defaults to <basedir>.')
    parser.add_argument('-q', '--quiet', action='store_true', help='Suppress progress output.')
    parser.add_argument('-v', '--verbose', action='store_true', help='Accepted for CLI symmetry; output is verbose by default.')
    ...
    run_cli(args.basedir, output_csv=args.output_csv, report_dir=args.report_dir, quiet=args.quiet)
